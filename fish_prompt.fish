...
#
  ...
  set -l branch_with_state
  if command hg id >/dev/null 2>&1
      set branch_with_state (command hg prompt "{branch}{status}")
      switch "$branch_with_state"
        case '*\!'
          set branch_with_state (command echo $branch_with_state | sed "s/\!/ ±/")
          set_color red; echo -n " ($branch_with_state)"
        case '*\?'  
          set branch_with_state (command echo $branch_with_state | sed "s/\?/ ±/")
          set_color red; echo -n " ($branch_with_state)"
        case '*'
          set_color green; echo -n " ($branch_with_state)"
      end      
  end
 end
